...
answers with sources and the tier that produced them, refusals when the
documents do not answer, contradictions refused until an admin resolves
them in `/manage`. Because the corpus is shared, the sidebar is the
company's single list — add a policy once, everyone can ask about it
seconds later.
...
2. **Folders as categories, permissions per folder.** Subfolders already
   index; next they become visible structure (grouped sidebar, admin
   organisation in /manage) and then ACL boundaries: HR/ readable by the
   HR group, mapped from Entra groups.
