...
class AnswerStore:
    ...
    def cost_report(self, since: float | None = None) -> dict[str, object]:
        ...
        with self._lock:
            ...
            rows = self._conn.execute(
                "SELECT tier, COUNT(*) AS n, SUM(cost_usd) AS spend"
                f" FROM ledger {where} GROUP BY tier",
                params,
            ).fetchall()

            by_tier = {
                r["tier"]: {"questions": r["n"], "spend_usd": round(r["spend"] or 0.0, 6)}
                for r in rows
            }
    ...
    def question_demand(self, limit: int = 20, since: float | None = None) -> list[QuestionDemand]:
        ...
        with self._lock:
            where, params = ("WHERE ts >= ?", (since,)) if since is not None else ("", ())
            rows = self._conn.execute(
                "SELECT canonical_query, tier, COUNT(*) AS n, SUM(cost_usd) AS spend,"
                " MAX(ts) AS last"
