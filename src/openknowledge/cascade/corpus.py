...
def recognise(question: str) -> CorpusQuestion | None:
    ...
    collection *and* has no other subject in it. "How many documents do you
    have" qualifies; "how many days of parental leave" does not, and neither
    does "which documents mention contractors".
    """
    words = _WORDS.findall(question.lower())
    if not words:
        return None
    if not (_META & set(words)):
        return None
    if any(word not in _META and word not in _EMPTY for word in words):
        return None
    return CorpusQuestion(wants="count" if "many" in words or "much" in words else "list")


def describe(titles: list[str], *, chunks: int, hidden: int = 0, limit: int = 40) -> str:
    """State what is indexed, in the words a person asked for it.

    Titles only, never contents: this path exists because it can answer without
    reading anything, and the moment it summarises what is *in* a document it is
    making a claim no gate has checked.
    """
    if not titles:
        return (
    ...
    lines = [f"I have {count} {noun} indexed, in {chunks} passage{'' if chunks == 1 else 's'}:"]
